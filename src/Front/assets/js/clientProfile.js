        ...
        <div class="card mb-4 bg-light border-0">
          <div class="card-body">
            ...
            <footer class="d-flex justify-content-between align-items-center">
              <button onclick="showInterestedFreelancersModal([${"'" + interestedArray.join("', '") + "'"}])" class="btn btn-indigo btn-sm w-30" ${job.interested.length < 1 ? "disabled" : ""}>
...
async function showInterestedFreelancersModal(interestedArray) {
  ...
  for (interested of interestedArray) {
    const { data } = await axios.get(`http://localhost:4000/auth/user/${interested}`, {
      headers: { token: localStorage.getItem("token") },
    });
    console.log(data);
    interestedList.innerHTML += `
    <li class="d-flex justify-content-between">
              <button class="btn">
                <span class="badge rounded-circle bg-primary me-2">${data.username.charAt(0).toUpperCase()}</span>
                ${data.username}
              </button>
